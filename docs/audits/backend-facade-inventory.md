...
| `sftp` | SFTP session adapter and remote operations | `shell`; internal SSH/transfer adapters | `labonair-sftp` and `labonair-transfers` integration boundaries; concrete transfer execution is owned by `labonair-transfers-ssh`, while SFTP service receives only SSH state plus the raw event bus and legacy connection orchestration receives EventBus instead of App |
