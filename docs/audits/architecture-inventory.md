...
This document records the current repository shape before the module migration. It is evidence for the rework; it is not a target design.
...
2. Extract platform services from `backend` without changing user behavior.
