...
- `shell/src/status_items.rs` still contains workspace-shell status behavior
  ...
  Hosts management entrypoint, updater Check-for-Updates, and Workspace panel
  surface actions are owner-registered. Remaining shell adapters are
  transitional until their owners move.
