...
| `backend` | MCP, snippet, and transfer service/event adapters | split across platform services and feature modules | Platform-adapter package; SSH transport now lives in `labonair-ssh-transport`, SFTP in `labonair-sftp-ssh`, Git in `labonair-git-transport`, and concrete transfer execution in `labonair-transfers-ssh`; broad application composition state, Settings migrations, and updater capability logic no longer live here. |
