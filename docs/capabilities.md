...
| Keymap | Redesign | `keymap` | `labonair-keymap` | dedicated keymap surface | keymap file and binding registry | Titlebar global menu → Keymap; quick access through palette | UI-free keymap crate and dedicated editor are active; shell retains only platform key installation/watch wiring |
| Command Palette | Redesign | `command-palette` | `labonair-command-palette-core` | `labonair-command-palette` | typed providers from feature modules | Titlebar global menu / global shortcut | Metadata, ordinary command handlers, and status-item registrations are owner-contributed; dynamic submenu action execution remains a documented R07-002 shell-composition gap |
...
| Themes (color and icon) | Redesign / static first | `themes` | `labonair-theme` | `labonair-theme` unless a real UI boundary requires a sibling crate | built-in definitions first | Titlebar global menu → Themes / Icon Themes → palette submenu | Separate app-theme and icon-theme palette pages; catalogs and preview state are owner-owned, while selection/preview event execution remains a documented R07-002 shell-composition gap; download/extension workflow remains deferred |
