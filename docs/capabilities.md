...
| Command Palette | Redesign | `command-palette` | `labonair-command-palette-core` | `labonair-command-palette` | typed providers from feature modules | Titlebar global menu / global shortcut | Metadata, submenu snapshots, and product command execution are owner-contributed; shell retains only native/debug composition cases |
