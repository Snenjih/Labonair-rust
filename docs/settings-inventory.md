...
| `appearance` | `tabsLocation`, `zenModeShowHeader`, `zenModeShowStatusbar` | shell/workspace presentation (`shell/src/titlebar.rs`, `shell/src/app_shell.rs`, `command-palette/src/palette.rs`); `tabsLocation == "sidebar"` moves the tab strip into the workspace-owned Tabs dock panel (`workspace/src/tabs_panel.rs`, reconciled by `Workspace::sync_tabs_in_sidebar`); Global | Keep |
...
| `terminal` | `terminalFontFamily`, `terminalFontSize`, `terminalLineHeight`, `terminalFontWeight`, `terminalScrollback`, `terminalCursorStyle`, `terminalCursorBlink`, `terminalCursorBlinkInterval`, `terminalCopyOnSelect`, `terminalRightClickPastes`, `terminalBell`, `terminalOpacity` | terminal renderer/settings adapter (`theme-ui` font-override bridge for family/size/line-height/weight; `workspace` terminal view for the rest — blink is rendered by `TerminalView`, live-tuned via `Workspace::reapply_terminal_settings`); Global | Keep |
| `terminal` | `sessionScrollbackLines`, `scrollbackMaxSizeMb`, `scrollbackRetentionDays` | scrollback persistence/cleanup; Global | Keep |
| `terminal` | `terminalWordSeparator`, `terminalScrollSensitivity`, `terminalFastScrollModifier`, `confirmCloseTerminalTab` | `workspace` terminal view — `terminalWordSeparator` feeds `alacritty_terminal`'s `semantic_escape_chars` for double-click selection; scroll sensitivity / fast-scroll modifier scale the wheel step; `confirmCloseTerminalTab` gates `Workspace::request_close`; Global | Keep |
| `terminal` | `terminalEnvironmentVariables`, `terminalShellArgs` | terminal/SSH shell spawn (`workspace/src/workspace.rs` `spawn_session` → `SessionOptions.env`/`.args`, consumed by `terminal/src/session.rs`); `KEY=value,…` / whitespace-separated `Text` values, parsed by `labonair_settings_content::terminal::{parse_environment_variables,parse_shell_args}`; Global | Keep |
| `terminal` | `terminalLetterSpacing` | no current Settings consumer; theme typography value is theme-owned | Remove; legacy input remains deserializable only |
| `terminal` | `terminalDefaultPath`, `newTabInheritsCwd`, `terminalShowPaneHeader`, `terminalShowPaneFooter` | no current native terminal consumer | Remove; legacy input remains deserializable only |
...
| `editor` | `editorHighlightCurrentLine`, `editorCursorBlink`, `editorCursorBlinkIntervalMs`, `editorCursorStyle` | native editor caret/line rendering (`workspace/src/views/editor.rs` render — current-line band, blink task mirroring the terminal view's, block/underline/bar caret shapes); Global | Keep |
...
| `fileManager` | `sftpShowHiddenFiles`, `sftpShowUpFolder`, `sftpZebraStriping`, `sftpRelativeTimes`, `sftpColumns` | `SftpBrowserSettings` — read by the dual-pane SFTP browser (`workspace/src/views/sftp.rs`), re-applied live via a `SettingsStore` observer; `sftpColumns` is the ordered visible-column list, editable in Settings (checkbox + reorder) and by dragging the browser's column headers | Keep as browser-chrome values; Global |
| `fileManager` | `sftpColumnSize`, `sftpColumnModified`, `sftpColumnPermissions`, `sftpColumnType`, `sftpFontSize` | superseded by `sftpColumns`; the four v1 visibility booleans fold into the ordered list on migration | Remove; legacy input remains deserializable only |
...
| `workspace` | `confirmQuitWithActiveSessions` | window-close gate (`shell/src/bootstrap.rs` `on_window_should_close`, checked against `labonair_workspace::Workspace::has_running_shells`); blocks the first quit attempt and pushes a notification, a second attempt within 3s proceeds; Global | Keep |
| `connections` | `sshConnectTimeoutSecs` | SSH handshake timeout (`workspace/src/workspace.rs` `spawn_ssh_connect`, `workspace/src/views/sftp.rs` `connect` → `SshConnectRequest.connect_timeout_secs`); Global | Keep |
| `connections` | `sshKeepaliveIntervalSecs`, `sshKeepaliveMaxFailures` | fallback SSH keep-alive applied only when a host record leaves its own `keep_alive_interval`/`keep_alive_tries` unset (`ssh-transport/src/client.rs` `ssh_connect`, via `SshConnectRequest.keepalive_interval_secs`/`.keepalive_max_failures`); does not affect quick-connect, jump hosts, or `ssh_test_connection`, which keep the transport's built-in 25s/3-failure default — extending those paths is future follow-up, not blocked on anything; Global | Keep |
