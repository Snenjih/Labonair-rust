...
3. **`impl Panel` je Panel-Crate**:
   ...
   - `-panel-hosts`: `"hosts"`, `Left`, `icon = Server`.
...
4. **Registrierung in `labonair-shell`**: eine Funktion
   ...
   ExplorerView::new(...))))` für alle sechs. Das ist der **einzige** Ort mit
   konkreten Panel-Typen.
5. **`app_shell.rs` umstellen**: `enum SidebarPanel` + `render_panel_body` +
   `SidebarPanel::{label,slug,from_slug}` löschen. `render_sidebar` liest
   `workspace.panel_registry()` + den aktuellen Dock-Zustand (Dock-Modell
   kommt in T17-002 — bis dahin einen minimalen „aktives Panel je Seite"-State
   im `Workspace` halten, den T17-002 durch `Dock` ersetzt).
6. **Persistenz-Schlüssel**: `sidebarActivePanel`/`sidebar…`-Prefs weiter über
   ...
   `explorer|snippets|source-control|hosts|ai`; `git-graph` neu). Migration
   trivial, in Doc-Kommentar notieren.
...
## Akzeptanzkriterien
- [ ] `enum SidebarPanel` und `render_panel_body` existieren nicht mehr.
- [ ] Alle sechs Panel-Crates haben ein `impl Panel`; `labonair-shell` hat
      genau eine `register_builtin_panels`-Stelle.
- [ ] `PanelRegistry` liefert Panels nach Name + Position; `render_sidebar`
      nutzt ausschließlich die Registry.
- [ ] `cargo run`: Panel-Toggles, Default-Seiten (Explorer/SCM/Hosts/Snippets
