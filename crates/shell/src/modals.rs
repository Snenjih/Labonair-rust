...
//! `labonair-panel-explorer` cannot depend on `labonair-workspace` (where
//! `ModalView` lives) without a cycle, and the orphan rule bars
//! `impl ModalView for CommandPalette` anywhere else.
...
pub(crate) struct BookmarksModal {
...
impl BookmarksModal {
    pub(crate) fn new(inner: Entity<BookmarksView>, cx: &mut Context<Self>) -> Self {
        let focus = inner.read(cx).focus_handle(cx);
        let dismiss = cx.subscribe(&inner, |_, _, _: &DismissEvent, cx| cx.emit(DismissEvent));
        Self {
            inner,
...
impl labonair_workspace::modal_layer::ModalView for BookmarksModal {
    fn on_dismiss(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
