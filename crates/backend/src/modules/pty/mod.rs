...
pub fn pty_open(
    ...
    shell: Option<String>,
    blocks: bool,
    on_event: crate::events::EventChannel<PtyEvent>,
) -> Result<u32, String> {
...
pub fn pty_write(state: &PtyState, id: u32, data: String) -> Result<(), String> {
    let session = state
        .sessions
        .read()
        .unwrap()
...
pub fn pty_resize(state: &PtyState, id: u32, cols: u16, rows: u16) -> Result<(), String> {
    let session = state
        .sessions
        .read()
        .unwrap()
...
#[cfg(unix)]
pub fn pty_has_foreground_job(state: &PtyState, id: u32) -> Result<bool, String> {
...
/// visible pane's own `crate::events::EventChannel<PtyEvent>`.
...
pub fn pty_close(state: &PtyState, id: u32) -> Result<(), String> {
