...
async fn write_to_ssh(app: &crate::App, session_id: &str, data: String) -> Result<(), String> {
    let state = &app.ssh;
...
async fn write_to_target(
    ...
    match kind {
        TerminalKind::Ssh => {
            write_to_ssh(
                app,
                session_id.ok_or("missing session_id for ssh target")?,
                data,
            )
            .await
        }
        TerminalKind::Local => {
            let pty_state = &app.pty;
            crate::modules::pty::write_raw(
                pty_state,
                local_pty_id.ok_or("missing local_pty_id for local target")?,
                &data,
            )
        }
    }
...
async fn capture_loop(
    ...
    loop {
        ...
        match tokio::time::timeout(remaining, recv).await {
            ...
            Ok(Err(_)) => {
                return Err(
                    "output stream interrupted (tab closed, or capture fell behind)".to_string(),
                );
            }
            Err(_elapsed) => {
                return Ok(TerminalExecResult {
...
pub async fn terminal_exec_run_command(
    ...
    app: crate::App,
    state: &TerminalExecState,
) -> Result<TerminalExecResult, String> {
    let lock_key = match kind {
        TerminalKind::Ssh => format!(
            "ssh:{}",
            session_id
                .as_deref()
                .ok_or("missing session_id for ssh target")?
        ),
        TerminalKind::Local => format!(
            "local:{}",
            local_pty_id.ok_or("missing local_pty_id for local target")?
        ),
    };
