...
async fn require_non_interactive_auth(app: &crate::App, host_id: &str) -> Result<(), String> {
    ...
    let auth_method = if let Some(cid) = &credential_id {
        let conn = hosts_db.0.lock().map_err(|e| e.to_string())?;
        conn.query_row(
            "SELECT cred_type FROM credentials WHERE id = ?1",
            rusqlite::params![cid],
            |r| r.get::<_, String>(0),
        )
...
impl LabonairMcpServer {
    ...
    ) -> Result<Json<ListSessionsResult>, String> {
    ...
    ) -> Result<Json<RunCommandResult>, String> {
        let grant = self
            .mcp_state
    ...
    async fn read_output(
    ...
    ) -> Result<Json<ReadOutputResult>, String> {
        let grant = self
            .mcp_state
            .grant_for_session(&params.session_id)
            .ok_or_else(|| {
        ...
        let deadline =
            std::time::Instant::now() + Duration::from_millis(params.wait_ms.unwrap_or(1000));
        let mut capture = Osc133Capture::new();
        loop {
            let remaining = deadline.saturating_duration_since(std::time::Instant::now());
            if remaining.is_zero() {
    ...
    async fn open_tab(
        ...
        let result = match tokio::time::timeout(Duration::from_secs(15), rx).await {
            ...
            _ => {
                ...
                return Err(
                    "timed out waiting for the app to open the tab (is Labonair running?)"
                        .to_string(),
                );
            }
        };
...
pub fn ensure_started(app: crate::App, mcp_state: McpState, token: String) {
    ...
    tokio::spawn(async move {
        ...
        let service: StreamableHttpService<LabonairMcpServer, LocalSessionManager> =
