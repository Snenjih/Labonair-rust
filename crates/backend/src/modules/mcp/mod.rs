...
impl McpState {
    ...
    fn grant_for_session(&self, session_id: &str) -> Option<SessionGrant> {
        ...
        map.values()
            .find(|g| g.session_id == session_id && g.granted)
            .cloned()
    }
...
pub async fn mcp_set_max_command_timeout_secs(secs: u64, state: &McpState) -> Result<(), String> {
    state
...
pub async fn mcp_set_auto_revoke_minutes(minutes: u32, state: &McpState) -> Result<(), String> {
