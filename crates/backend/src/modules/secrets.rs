...
fn plain_path(_app: &crate::App) -> Result<PathBuf, String> {
    Ok(crate::modules::fs::paths::data_dir().join(PLAIN_FILE))
}

fn enc_path(_app: &crate::App) -> Result<PathBuf, String> {
    Ok(crate::modules::fs::paths::data_dir().join(ENC_FILE))
}

fn key_path(_app: &crate::App) -> Result<PathBuf, String> {
    Ok(crate::modules::fs::paths::data_dir().join(KEY_FILE))
}

fn flag_path(_app: &crate::App) -> Result<PathBuf, String> {
    Ok(crate::modules::fs::paths::data_dir().join(ENC_FLAG_FILE))
}

// ── Encryption-key lifecycle ─────────────────────────────────────────────────

fn load_or_create_key(app: &crate::App) -> Result<[u8; 32], String> {
...
fn get_enc_key(app: &crate::App, state: &SecretsState) -> Result<[u8; 32], String> {
...
fn is_encryption_enabled(app: &crate::App) -> bool {
    flag_path(app).map(|p| p.exists()).unwrap_or(false)
}

fn read_map(app: &crate::App, state: &SecretsState) -> Result<HashMap<String, String>, String> {
...
fn with_cache<F, R>(app: &crate::App, state: &SecretsState, f: F) -> Result<R, String>
where
...
pub(crate) fn migrate_service_names(app: &crate::App, state: &SecretsState) {
    let renames = [
    ...
    let Ok(mut map) = read_map(app, state) else {
        return;
    };
