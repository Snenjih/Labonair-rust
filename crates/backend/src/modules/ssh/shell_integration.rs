...
mod tests {
    ...
    fn balances_heredoc_delimiters() {
        ...
        let closes = script.lines().filter(|l| l.trim() == HEREDOC_DELIM).count();
        assert_eq!(opens, 5);
        assert_eq!(closes, 5);
    }
    ...
    fn blocks_flag_exports_env_var_before_the_case_statement() {
        ...
        assert!(build_bootstrap_script(false)
            .starts_with("export COLORTERM=truecolor\ncase \"$SHELL\" in"));
    }
