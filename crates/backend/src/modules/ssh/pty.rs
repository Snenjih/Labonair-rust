    ...
    tokio::spawn(async move {
        ...
        'reader: loop {
            ...
            if !pending_output.is_empty() {
                let should_flush =
                    pending_output.len() >= SSH_BATCH_BYTES || last_flush.elapsed() >= SSH_BATCH_MS;
                if should_flush {
...
mod tests {
    ...
    fn humanize_disconnect_reason_maps_known_patterns() {
        assert!(humanize_disconnect_reason("transport read error")
            .contains("Network transport failure"));
        assert_eq!(
            humanize_disconnect_reason("Connection reset by peer"),
            "Connection reset by the server"
        );
        assert_eq!(
            humanize_disconnect_reason("broken pipe"),
            "Connection interrupted — the pipe to the server was broken"
        );
        assert_eq!(
            humanize_disconnect_reason("operation timed out"),
            "Connection timed out"
        );
        assert_eq!(
            humanize_disconnect_reason("network is unreachable"),
            "Network unreachable — no route to host"
        );
        assert_eq!(
            humanize_disconnect_reason("no route to host"),
            "Network unreachable — no route to host"
        );
        assert_eq!(
            humanize_disconnect_reason("connection refused"),
            "Connection refused by the server"
        );
        assert_eq!(
            humanize_disconnect_reason("unexpected eof"),
    ...
    fn humanize_disconnect_reason_capitalizes_unknown_errors() {
        assert_eq!(
            humanize_disconnect_reason("something weird happened"),
