...
pub async fn enqueue_transfer(
    ...
    log::info!(
        "[sftp] enqueue_transfer tab={} direction={} src={} dest={}",
        session_id,
        direction,
        src_path,
        dest_path
...
pub async fn cancel_transfer(job_id: String, worker: &TransferWorkerState) -> Result<(), String> {
    worker
        .sender
...
pub async fn sftp_session_reconnected(
    ...
    worker
        .sender
        .send(WorkerMessage::SessionReconnected(session_id))
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn sftp_update_transfer_settings(
    ...
    if let Some(v) = max_concurrent {
        worker
            .settings
            .max_concurrent
            .store(v.clamp(1, 16), Ordering::Relaxed);
    }
    if let Some(v) = chunk_size_bytes {
        worker
            .settings
            .chunk_size
            .store(v.clamp(4096, 8 * 1024 * 1024), Ordering::Relaxed);
    }
