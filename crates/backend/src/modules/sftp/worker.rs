...
async fn compute_local_md5_async(
...
async fn compute_remote_md5(session: &RushSession, remote_path: &str) -> Option<String> {
    ...
    channel
        .exec(true, format!("md5sum '{}'", escaped))
        .await
        .ok()?;
    ...
    loop {
        ...
        loop {
            match rx.try_recv() {
                ...
                Ok(WorkerMessage::ResolveConflict {
                    job_id,
                    resolution,
                    new_name,
                }) => {
        ...
        if in_flight.is_empty() {
            ...
            match rx.recv().await {
                ...
                Some(WorkerMessage::ResolveConflict {
                    job_id,
                    resolution,
                    new_name,
                }) => {
...
async fn walk_remote_tree(
    ...
    let mut stack: Vec<(String, String)> =
        vec![(String::new(), root.trim_end_matches('/').to_string())];
    ...
    let mut visited_symlink_targets: std::collections::HashSet<String> =
...
async fn download_file(
    ...
    log::debug!(
        "[sftp/download] looking up SFTP session for session_id={}",
        job.session_id
    );
    ...
    log::debug!(
        "[sftp/download] attempting atomic local create: {}",
        job.dest_path
    );
    emit_step(
        app,
        ...
        .await
    {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::AlreadyExists => {
            ...
            match resolution.resolution.as_str() {
                "skip" => {
                    log::debug!("[sftp/download] skipped by user");
                    return Ok(());
                }
                "rename" => {
    ...
    match compute_remote_md5(&session, &job.src_path).await {
        ...
        None => {
            log::warn!(
                "[sftp/download] md5sum unavailable on remote, relying on size check id={}",
                job.id
            );
            emit_step(
                app,
...
async fn download_directory(
    ...
    if tokio::fs::metadata(&dest_root).await.is_ok() {
        ...
        match resolution.resolution.as_str() {
            "skip" => {
                log::debug!("[sftp/download] folder skipped by user");
                return Ok(());
            }
            "rename" => {
                let new_name = resolution.new_name.ok_or("rename requires new_name")?;
    ...
    for entry in &entries {
        ...
        if let Err(e) = result {
            if skip_all_remaining || on_error_policy == "skip" {
                log::warn!(
                    "[sftp/download] skipping failed file {}: {e}",
                    entry.rel_path
                );
                emit_step(
                    app,
    ...
    emit_step(
        ...
        format!(
            ...
            if job.skipped_count > 0 {
                format!(", {} skipped", job.skipped_count)
            } else {
                String::new()
            },
        ),
...
async fn download_one_file(
    ...
    if local_size != expected_size {
        return Err(format!(
            "Size mismatch (remote={expected_size}, local={local_size})"
        ));
    }
    Ok(())
}
...
async fn upload_file(
    ...
    log::debug!(
        "[sftp/upload] looking up SFTP session for session_id={}",
        job.session_id
    );
    let (session, sftp) = get_session_and_sftp(ssh_state, &job.session_id)?;
    emit_step(app, &job.id, "SFTP session ready");
    ...
    log::debug!(
        "[sftp/upload] attempting atomic remote create: {}",
        job.dest_path
    );
    emit_step(
        app,
        ...
        .await
    {
        ...
        Err(_) => {
            ...
            match resolution.resolution.as_str() {
                "skip" => {
                    log::debug!("[sftp/upload] skipped by user");
                    return Ok(());
                }
                "rename" => {
                    let new_name = resolution.new_name.ok_or("rename requires new_name")?;
    ...
    match compute_remote_md5(&session, &job.dest_path).await {
        ...
        None => {
            log::warn!(
                "[sftp/upload] md5sum unavailable on remote, relying on size check id={}",
                job.id
            );
            emit_step(
                app,
...
async fn upload_directory(
    ...
    if sftp.metadata(dest_root.clone()).await.is_ok() {
        ...
        match resolution.resolution.as_str() {
            "skip" => {
                log::debug!("[sftp/upload] folder skipped by user");
                return Ok(());
            }
            "rename" => {
                let new_name = resolution.new_name.ok_or("rename requires new_name")?;
...
async fn upload_one_file(
    ...
    if remote_size != expected_size {
        return Err(format!(
            "Size mismatch (local={expected_size}, remote={remote_size})"
        ));
    }
    Ok(())
}
