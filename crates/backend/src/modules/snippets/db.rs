...
pub async fn snippets_get_all(db: &HostsDb) -> Result<Vec<CommandSnippet>, LabonairError> {
    let conn =
        db.0.lock()
...
pub async fn snippets_delete(db: &HostsDb, id: String) -> Result<(), LabonairError> {
    let conn =
        db.0.lock()
            .map_err(|e| LabonairError::Internal(e.to_string()))?;
...
pub async fn snippet_groups_get_all(db: &HostsDb) -> Result<Vec<SnippetGroup>, LabonairError> {
    let conn =
        db.0.lock()
            .map_err(|e| LabonairError::Internal(e.to_string()))?;
    let mut stmt = conn
...
pub async fn snippet_groups_delete(db: &HostsDb, id: String) -> Result<(), LabonairError> {
    let conn =
        db.0.lock()
            .map_err(|e| LabonairError::Internal(e.to_string()))?;
    conn.execute(
...
mod tests {
    ...
    fn delete_snippet_group_nulls_snippet_group_id() {
        ...
        .unwrap();
        conn.execute("UPDATE snippets SET group_id=NULL WHERE group_id='sg1'", [])
            .unwrap();
        conn.execute("DELETE FROM snippet_groups WHERE id='sg1'", [])
            .unwrap();
        let group_id: Option<String> = conn
            .query_row("SELECT group_id FROM snippets WHERE id='s1'", [], |r| {
                r.get(0)
            })
            .unwrap();
