...
pub async fn hosts_duplicate(
    ...
    if let Some(pw) =
        get_password(&app, secrets, "labonair-app", &id).map_err(LabonairError::Internal)?
    {
        store_password(&app, secrets, "labonair-app", &new_id, &pw)
            .map_err(LabonairError::Internal)?;
    }
    if let Some(sp) =
        get_password(&app, secrets, "labonair-sudo", &id).map_err(LabonairError::Internal)?
    {
        store_password(&app, secrets, "labonair-sudo", &new_id, &sp)
            .map_err(LabonairError::Internal)?;
    }
...
pub async fn hosts_get_all(db: &HostsDb) -> Result<Vec<Host>, LabonairError> {
    let conn =
        db.0.lock()
            .map_err(|e| LabonairError::Internal(e.to_string()))?;
    let mut stmt = conn.prepare(&format!(
        "{} ORDER BY pin_to_top DESC, sort_order ASC, name ASC",
        SELECT_HOSTS
    ))?;
    let hosts = stmt
...
pub async fn hosts_create(
    ...
    if let Some(pw) = password {
        if !pw.is_empty() {
            store_password(&app, secrets, "labonair-app", &id, &pw)
                .map_err(LabonairError::Internal)?;
        }
    }
    if let Some(sp) = sudo_password {
        if !sp.is_empty() {
            store_password(&app, secrets, "labonair-sudo", &id, &sp)
                .map_err(LabonairError::Internal)?;
        }
    }
    let conn =
...
pub async fn hosts_update(
...
) -> Result<Host, LabonairError> {
    {
        ...
        if private_key_path.is_some() {
            conn.execute(
                ...
                rusqlite::params![private_key_path, id],
            )?;
        }
        if group_id.is_some() {
            conn.execute(
    ...
    if block_agent_access == Some(true) {
        let mcp_state = &app.mcp;
        let expired: Vec<String> = {
            let grants = mcp_state
                .grants
                .lock()
                .map_err(|e| LabonairError::Internal(e.to_string()))?;
            grants
...
pub async fn hosts_reorder(db: &HostsDb, items: Vec<ReorderItem>) -> Result<(), LabonairError> {
    let conn =
        db.0.lock()
            .map_err(|e| LabonairError::Internal(e.to_string()))?;
    for item in &items {
...
pub async fn groups_get_all(db: &HostsDb) -> Result<Vec<Group>, LabonairError> {
    let conn =
        db.0.lock()
            .map_err(|e| LabonairError::Internal(e.to_string()))?;
    let mut stmt =
        conn.prepare("SELECT id, name, icon, color, created_at FROM groups ORDER BY name")?;
    let groups = stmt
        .query_map([], |row| {
            Ok(Group {
                id: row.get(0)?,
...
pub async fn groups_delete(db: &HostsDb, id: String) -> Result<(), LabonairError> {
    let conn =
        db.0.lock()
            .map_err(|e| LabonairError::Internal(e.to_string()))?;
...
pub async fn groups_update(db: &HostsDb, id: String, name: String) -> Result<Group, LabonairError> {
    let conn =
        db.0.lock()
            .map_err(|e| LabonairError::Internal(e.to_string()))?;
    conn.execute(
...
mod tests {
    ...
    fn schema_init_is_idempotent() {
        ...
        conn.execute_batch(schema)
            .expect("second run should not panic");
    }
}
