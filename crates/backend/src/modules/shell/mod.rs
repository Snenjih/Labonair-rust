...
pub async fn shell_run_command(
    ...
    let ceiling = max_timeout_secs
        .unwrap_or(MAX_TIMEOUT_SECS)
        .min(HARD_MAX_TIMEOUT_SECS);
...
pub fn shell_session_open(state: &ShellState, cwd: Option<String>) -> Result<u32, String> {
...
pub fn shell_session_close(state: &ShellState, id: u32) -> Result<(), String> {
...
pub fn shell_bg_kill(state: &ShellState, handle: u32) -> Result<(), String> {
...
pub fn shell_bg_list(state: &ShellState) -> Result<Vec<BackgroundProcInfo>, String> {
