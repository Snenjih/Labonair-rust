...
impl GitExecutor {
    ...
    pub(crate) async fn run_raw_tolerant(
        &self,
        args: &[&str],
        tolerated: &[i32],
    ...
    pub(crate) async fn run_with_stdin(
        ...
        match self {
            GitExecutor::Local { cwd } => run_local_with_stdin(args, cwd, &stdin_bytes)
                .await
                .map(|_| ()),
            GitExecutor::Remote {
                session_id,
    ...
    async fn exec_remote_args(&self, args: &[&str]) -> Result<(Vec<u8>, Vec<u8>, i32), String> {
        ...
        let quoted_args: String = args
            ...
            .join(" ");
        let cwd_quoted = shell_quote(cwd);
        let build_script = |git_bin: &str| {
            format!("LC_ALL=C GIT_TERMINAL_PROMPT=0 {git_bin} -C {cwd_quoted} {quoted_args}")
        };
