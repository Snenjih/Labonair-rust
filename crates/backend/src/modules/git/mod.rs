...
fn parse_submodule_status(output: &str) -> Vec<SubmoduleStatus> {
    ...
    for line in output.lines() {
        ...
        let path = path_and_describe
            .split(" (")
            .next()
            .unwrap_or(path_and_describe)
            .trim()
            .to_string();
...
fn parse_branches(output: &str) -> Vec<Branch> {
    ...
    for line in output.lines() {
        ...
        let committed_relative = if committed_relative_raw.is_empty() {
            None
        } else {
            Some(committed_relative_raw)
        };
...
pub async fn git_get_status(
    ...
    let ahead_behind_str = sections
        .get(1)
        .map(|b| String::from_utf8_lossy(b).into_owned())
        .unwrap_or_default();
    let flags_str = sections
        .get(2)
        .map(|b| String::from_utf8_lossy(b).into_owned())
        .unwrap_or_default();
    ...
    let (merge_in_progress, rebase_in_progress, cherry_pick_in_progress) =
        parse_state_flags(&flags_str);
...
async fn apply_hunk_patch(
    ...
    executor
        .run_with_stdin(&args, hunk_patch_text.into_bytes())
        .await
        .map_err(|e| classify_apply_error(&e, file))
}
...
pub async fn git_get_workspace_state(
    ...
    let porcelain = sections.first().copied().unwrap_or(&[]);
    let branches_str = sections
        .get(1)
        .map(|b| String::from_utf8_lossy(b).into_owned())
        .unwrap_or_default();
    let stash_str = sections
        .get(2)
        .map(|b| String::from_utf8_lossy(b).into_owned())
        .unwrap_or_default();
    let tags_str = sections
        .get(3)
        .map(|b| String::from_utf8_lossy(b).into_owned())
        .unwrap_or_default();
    ...
    let flags_str = sections
        ...
        .map(|b| String::from_utf8_lossy(b).into_owned())
        .unwrap_or_default();
    let show_current_str = sections
        .get(7)
        .map(|b| String::from_utf8_lossy(b).into_owned())
        .unwrap_or_default();
    let short_hash_str = sections
        .get(8)
        .map(|b| String::from_utf8_lossy(b).into_owned())
        .unwrap_or_default();
    let submodule_status_str = sections
        .get(9)
        .map(|b| String::from_utf8_lossy(b).into_owned())
        .unwrap_or_default();
    ...
    let staged_numstat = diffstats_parts
        .first()
        .map(|b| String::from_utf8_lossy(b).into_owned())
        .unwrap_or_default();
    let unstaged_numstat = diffstats_parts
        .get(1)
        .map(|b| String::from_utf8_lossy(b).into_owned())
        .unwrap_or_default();
...
pub async fn git_add_to_gitignore(
    ...
    if session_id.is_none() {
        ...
        if existing
            .lines()
            .any(|l| l.trim() == entry.trim() || l.trim() == safe_file.trim())
        {
            return Ok(());
        }
    ...
    if exit_code != 0 {
        return Err(format!(
            "adding .gitignore entry failed (exit code {exit_code})"
        ));
    }
    Ok(())
}
...
pub async fn git_add_to_exclude(
    ...
    if exit_code != 0 {
        return Err(format!(
            "adding .git/info/exclude entry failed (exit code {exit_code})"
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn shortstat_parses_all_fields() {
        assert_eq!(
            parse_shortstat("3 files changed, 10 insertions(+), 4 deletions(-)"),
            (3, 10, 4)
        );
    }
    ...
    fn stash_gs_parses_wip_form() {
        assert_eq!(
            parse_stash_gs("WIP on main: abc123 fix stuff"),
            ("main".to_string(), "abc123 fix stuff".to_string())
        );
    }
    ...
    fn stash_gs_parses_on_form() {
        assert_eq!(
            parse_stash_gs("On feature/x: my message"),
            ("feature/x".to_string(), "my message".to_string())
        );
    }
    ...
    fn stash_gs_falls_back_when_unrecognized() {
        assert_eq!(
            parse_stash_gs("some custom message"),
            (String::new(), "some custom message".to_string())
        );
    }
...
? untracked.txt\0";
    ...
    fn porcelain_status_v2_uses_dot_not_space_for_unchanged_side() {
        ...
        assert_eq!(
            staged.len(),
            0,
            "an unstaged-only change must not also appear staged"
        );
    ...
    fn porcelain_status_v2_parses_submodule_dirty_worktree() {
        ...
        let sub = unstaged[0]
            .submodule
            .as_ref()
            .expect("submodule field present");
...
+5b276bbf6fd8afd8b889df24e4eb2428c3f5c7ee sub (heads/main)\n\
 9533341af956c9055329769452f2bf84f386b8fe sub (heads/main)";
        ...
        assert_eq!(
            statuses[0].commit,
            "9533341af956c9055329769452f2bf84f386b8fe"
        );
        assert_eq!(statuses[1].state, SubmoduleSyncState::PointerChanged);
        assert_eq!(
            statuses[1].commit,
            "5b276bbf6fd8afd8b889df24e4eb2428c3f5c7ee"
        );
    ...
    fn branches_parses_current_and_upstream_tracking() {
        // Fields are NUL-separated (see BRANCH_FORMAT's `%00`), one branch per line.
        let raw = "main\0*\0origin/main\0ahead 2, behind 1\nfeature\0.\0.\0";
    ...
    fn stash_list_parses_multiple_entries_separated_by_record_sep() {
        let raw =
            "stash@{0}\0WIP on main: abc first\0hash1\x1estash@{1}\0On dev: second\0hash2\x1e";
    ...
    fn split_first_hunk(raw_diff: &str) -> (String, String) {
        ...
        let first_hunk_idx = lines
            ...
            .expect("no hunk header found");
        let second_hunk_idx = lines[first_hunk_idx + 1..]
            .iter()
            .position(|l| l.starts_with("@@ -"))
            .map(|i| i + first_hunk_idx + 1);
    ...
    async fn stage_and_unstage_single_hunk_via_apply_cached() {
        ...
        assert!(
            raw_diff.matches("@@ -").count() >= 2,
            "expected two hunks, got diff:\n{raw_diff}"
        );
        let (header, hunk1) = split_first_hunk(&raw_diff);
        let patch = format!("{header}\n{hunk1}\n");

        let executor = GitExecutor::Local {
            cwd: dir.to_string_lossy().into_owned(),
        };

        // Stage only hunk 1 (the a2 change).
        apply_hunk_patch(&executor, "tracked.txt", patch.clone(), false)
            .await
            .expect("stage hunk 1");

        let staged = git_output(&dir, &["diff", "--cached", "--", "tracked.txt"]);
        assert!(
            staged.contains("a2_CHANGED"),
            "staged diff missing hunk 1:\n{staged}"
        );
        assert!(
            !staged.contains("a14_CHANGED"),
            "staged diff leaked hunk 2:\n{staged}"
        );
        ...
        apply_hunk_patch(&executor, "tracked.txt", patch, true)
            .await
            .expect("unstage hunk 1");
        let staged_after_unstage = git_output(&dir, &["diff", "--cached", "--", "tracked.txt"]);
        assert!(
            staged_after_unstage.is_empty(),
            "expected nothing staged after unstaging hunk 1:\n{staged_after_unstage}"
        );
