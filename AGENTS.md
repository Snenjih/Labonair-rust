...
2. Write memory entries to `/memory/` for progress and any bugs/decisions.
3. Commit all changes with a descriptive conventional commit message.
4. Record any error/debug you encountered in `/memory/bugs_and_fixes.md` (what failed, why, how fixed).

## Bug & Error Memory Rule

If you hit a build error, unexpected behavior, or had to debug something non-obvious:
→ Write a memory entry in `/memory/bugs_and_fixes.md`
