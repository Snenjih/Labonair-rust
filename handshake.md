...
The updater `CheckForUpdates` command was moved into
`labonair-updater-ui::command_provider` in `88e2846`. Its metadata and
executable handler are now owned by the updater UI sibling; bootstrap creates
the `UpdaterView` before command/keymap composition and injects that entity
into the owner registration. The shell command table no longer contains
...
Workspace surface command handlers were moved into
`labonair-workspace::command_provider` in `f7c643e`. Sidebar toggling,
dock cycling/zoom, Snippets and Source Control panel focus, and Git Graph tab
opening now operate through the Workspace owner entity; the shell no longer
contains those layout/panel closures or helper methods. Targeted
...
Legacy `appCornerRadius` values are converted to the current
`cornerRadiusScale` field during migration, with the modern value taking
precedence when both are present.

Current HEAD is `b83b4a9` on `master`; the worktree contains this handshake
...
MCP bridge preferences are now owned by
`labonair_mcp_core::preferences` with explicit path-based load/save methods;
shell composition supplies the config directory and the backend Settings MCP
module was deleted. Full workspace tests, Clippy, dependency, queue, and diff
checks pass.
The unreferenced backend copies of agents, directives, and model preferences
were deleted after repository-wide consumer search; no reachable workflow was
...
The Git operation surface now receives explicit `EventBus` values, and both
Git service adapters retain only `SshState` plus `EventBus`.
SSH PTY operations now use a dedicated `SshState` adapter, and remote
command/file operations use a dedicated `SshState + EventBus` adapter.
Secrets compatibility wrappers and jump-host resolution now receive only
...
is the remaining backend worker state and compatibility paths.
The MCP HTTP server now uses an explicit `McpServerAccess` bundle instead of
...
Command descriptors now own typed default bindings through
`with_default_binding`. Keymap materializes those defaults into the default
...
focused Keymap/Shell tests, Clippy, format, dependency graph, and full
workspace test suite pass; the parallel suite's transient SQLite lock passed
when rerun in isolation. Current commit: `c2bffcf`; worktree is clean.
...
The current branch is `master` at `ddc2289`; this partial migration is
committed locally.
...
`SshCommandExecutor`; the backend provides `BackendSshExecutor` as the only
russh/session adapter. `SnippetsView` receives that capability during shell
...
credentials capability is committed as `734c393` and the snippet boundary as
`d02e7ef`. The full
...
extracting snippet process execution from the backend and continuing the typed
service/event migration. No source blocker.
